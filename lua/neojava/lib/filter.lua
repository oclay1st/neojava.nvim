...
M.show_filter = function(state, search_as_you_type, fuzzy_finder_mode, use_fzy)
  ...
  local input = Input(popup_options, {
    ...
    on_submit = function(value)
      if value == '' then
        neojava.reset_search(state)
      else
        if search_as_you_type and fuzzy_finder_mode then
          neojava.reset_search(state, true, true)
          return
        end
        state.search_pattern = value
        manager.refresh('neojava', function()
          -- focus first file
    ...
    on_change = function(value)
      ...
      elseif value == '' then
        ...
        utils.debounce('neojava_filter', function()
          neojava._navigate_internal(state, nil, nil, callback)
