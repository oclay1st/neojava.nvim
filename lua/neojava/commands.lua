...
local filter = require('neojava.lib.filter')
...
local refresh = function(state)
  neojava._navigate_internal(state, nil, nil, nil, false)
end
...
M.add = function(state)
  ...
  if neojava_core_utils.is_inside_java_package(path) then
    neojava_core.create_artifact(path, function(artifact_path)
      neojava.navigate(state, state.path, artifact_path, function()
        if state.tree:get_node():get_id() == artifact_path then
          M.open(state)
        end
      end)
    end)
  else
    cc.add(state, utils.wrap(neojava.show_new_children, state))
  end
end

M.add_directory = function(state)
  cc.add_directory(state, utils.wrap(neojava.show_new_children, state))
end

M.clear_filter = function(state)
  neojava.reset_search(state, true)
end

M.copy = function(state)
  cc.copy(state, utils.wrap(neojava.focus_destination_children, state))
end
...
M.move = function(state)
  cc.move(state, utils.wrap(neojava.focus_destination_children, state))
end

---Pastes all items from the clipboard to the current directory.
M.paste_from_clipboard = function(state)
  cc.paste_from_clipboard(state, utils.wrap(neojava.show_new_children, state))
end
...
M.expand_all_nodes = function(state, node)
  ...
  cc.expand_all_nodes(state, node, neojava.prefetcher)
end
...
M.navigate_up = function(state)
  ...
  if state.search_pattern then
    neojava.reset_search(state, false)
  end
  log.debug('Changing directory to:', parent_path)
  neojava._navigate_internal(state, parent_path, path_to_reveal, nil, false)
end
...
local focus_next_git_modified = function(state, reverse)
  ...
  if existing then
    ...
    neojava.navigate(state, state.path, target, nil, false)
  end
end
...
M.open_rightbelow_vs = function(state)
  cc.open_rightbelow_vs(state, utils.wrap(neojava.toggle_directory, state))
end
M.open_leftabove_vs = function(state)
  cc.open_leftabove_vs(state, utils.wrap(neojava.toggle_directory, state))
end
M.open_vsplit = function(state)
  cc.open_vsplit(state, utils.wrap(neojava.toggle_directory, state))
end
M.open_tabnew = function(state)
  cc.open_tabnew(state, utils.wrap(neojava.toggle_directory, state))
end
M.open_drop = function(state)
  cc.open_drop(state, utils.wrap(neojava.toggle_directory, state))
end
M.open_tab_drop = function(state)
  cc.open_tab_drop(state, utils.wrap(neojava.toggle_directory, state))
end

M.open_with_window_picker = function(state)
  cc.open_with_window_picker(state, utils.wrap(neojava.toggle_directory, state))
end
M.split_with_window_picker = function(state)
  cc.split_with_window_picker(state, utils.wrap(neojava.toggle_directory, state))
end
M.vsplit_with_window_picker = function(state)
  cc.vsplit_with_window_picker(state, utils.wrap(neojava.toggle_directory, state))
end
...
M.set_root = function(state)
  ...
  neojava._navigate_internal(state, node:get_id(), nil, nil, false)
end
...
M.toggle_node = function(state)
  cc.toggle_node(state, utils.wrap(neojava.toggle_directory, state))
end
